...
def run_AI_all(
    ...
    if len(data[0]) < 3:
        ...
        "results/NN_trained_models/models/" + filename + ".h5"
    ):  # or len(data[0])<3:
        print("NN already trained \n")
        print("NN loss: ", NN_eval(pathdir, filename)[0], "\n")
        model_feynman = NN_eval(pathdir, filename)[1]
    elif path.exists(
        "results/NN_trained_models/models/" + filename + "_pretrained.h5"
    ):
        print("Found pretrained NN \n")
        model_feynman = NN_train(
            pathdir,
            filename,
            NN_epochs / 2,
            lrs=1e-3,
            N_red_lr=3,
            pretrained_path="results/NN_trained_models/models/"
            + filename
            + "_pretrained.h5",
        )
    ...
    if symmetry_plus_result[0] == -1:
        ...
        idx_min = np.argmin(
            np.array(
                [
                    (
                        symmetry_plus_result[0].cpu().numpy()
                        if isinstance(symmetry_plus_result[0], torch.Tensor)
                        else symmetry_plus_result[0]
                    ),
                    (
                        symmetry_minus_result[0].cpu().numpy()
                        if isinstance(symmetry_minus_result[0], torch.Tensor)
                        else symmetry_minus_result[0]
                    ),
                    (
                        symmetry_multiply_result[0].cpu().numpy()
                        if isinstance(
                            symmetry_multiply_result[0], torch.Tensor
                        )
                        else symmetry_multiply_result[0]
                    ),
                    (
                        symmetry_divide_result[0].cpu().numpy()
                        if isinstance(symmetry_divide_result[0], torch.Tensor)
                        else symmetry_divide_result[0]
                    ),
                    (
                        separability_plus_result[0].cpu().numpy()
                        if isinstance(
                            separability_plus_result[0], torch.Tensor
                        )
                        else separability_plus_result[0]
                    ),
                    (
                        separability_multiply_result[0].cpu().numpy()
                        if isinstance(
                            separability_multiply_result[0], torch.Tensor
                        )
                        else separability_multiply_result[0]
                    ),
                ]
            )
        )
    ...
    if len(data[0]) > 3:
        ...
        brute_force_gen_sym(
            "results/", "gradients_gen_sym_%s" % filename, 600, "14ops.txt"
        )
        ...
        for bf_i in range(len(bf_all_output)):
            ...
            try:
                express = bf_all_output[:, 1][bf_i]
                idx_gen_sym_temp, eqq, new_mu, new_sigma = check_gen_sym(
                    pathdir,
                    filename,
                    model_feynman,
                    decomp_idx,
    ...
    if idx_min == 0:
        print(
            "Translational symmetry found for variables:",
            symmetry_plus_result[1],
            symmetry_plus_result[2],
        )
        new_pathdir, new_filename = do_translational_symmetry_plus(
            pathdir, filename, symmetry_plus_result[1], symmetry_plus_result[2]
        )
        PA1_ = ParetoSet()
        PA1 = run_AI_all(
            new_pathdir,
        ...
        print(
            "Additive separability found for variables:",
        ...
        print(
            "Multiplicative separability found for variables:",
        ...
        new_pathdir, new_filename = do_compositionality(
            pathdir, filename, math_eq_comp
        )
        PA1_ = ParetoSet()
        PA1 = run_AI_all(
            new_pathdir,
        ...
        new_pathdir, new_filename = do_gen_sym(
            pathdir, filename, decomp_idx, math_eq_gen_sym
        )
        PA1_ = ParetoSet()
        PA1 = run_AI_all(
            new_pathdir,
...
def run_aifeynman(
    ...
    train_data = input_data[
        sep_idx[0 : (100 - test_percentage) * len(input_data) // 100]
    ]
    test_data = input_data[
        sep_idx[test_percentage * len(input_data) // 100 : len(input_data)]
    ]
    ...
    np.savetxt(
        "results/solution_before_snap_%s.txt" % filename, PA_list, fmt="%s"
    )
    ...
    PA_list = PA.get_pareto_points()
    np.savetxt(
        "results/solution_first_snap_%s.txt" % filename, PA_list, fmt="%s"
    )
    ...
    PA_list = PA.get_pareto_points()
    for j in range(len(PA_list)):
        PA = add_snap_expr_on_pareto(
            pathdir, filename, PA_list[j][-1], PA, DR_file
        )
    ...
    if DR_file == "" and test_data.size != 0:
        ...
        for i in range(len(list_dt)):
            test_errors = test_errors + [
                get_symbolic_expr_error(input_test_data, str(list_dt[i][-1]))
            ]
